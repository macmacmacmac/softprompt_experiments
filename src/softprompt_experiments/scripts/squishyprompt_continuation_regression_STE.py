...
def run(args_list):
    ...
    for dataset_dir in tqdm(dataset_dirs):
        ...
        def ste_generate(
            ...
            # assume we predict z′ tokens from the last max_new_tokens positions
            logits = outputs.logits[:, -max_new_tokens:, :]  # (B, Tz, V)
            ...
            y = one_hot + probs - probs.detach()   # STE trick

            # --- embed tokens ---
            zprime_embeds = y @ embedding_matrix   # (B, Tz, D)

            return (
                zprime_embeds,   # (B, Tz, D)
                y                # (B, Tz, V)
            )

        def prep_inputs_for_zprime_loss_embeds(
            input_ids_x_y,
            labels_x_y,
            zprime_embeds,   # (B, Tz, D) from gumbel_generate
        ):
            B = input_ids_x_y.size(0)
            ...
            for i in range(B):
                ...
                embed_x = word_embeddings(input_id_x)
                embed_y = word_embeddings(input_id_y)

                # --- get z' embeds (already no padding!) ---
                embed_z = zprime_embeds[i]  # (Tz, D)

                # --- concat ---
                full_embeds = torch.cat([embed_x, embed_z, embed_y], dim=0)

                # --- attention mask ---
                attn = torch.ones(full_embeds.size(0), device=device)

                # --- labels ---
                labels = torch.cat([
                    torch.full((embed_x.size(0),), -100, device=device),
                    torch.full((embed_z.size(0),), -100, device=device),
                    input_id_y
                ], dim=0)

                list_embeds.append(full_embeds)
                list_attn.append(attn)
                list_labels.append(labels)

            # --- pad manually (important!) ---
            max_len = max(x.size(0) for x in list_embeds)

            padded_embeds = []
            padded_attn = []
            padded_labels = []

            for embeds, attn, labels in zip(list_embeds, list_attn, list_labels):
                pad_len = max_len - embeds.size(0)

                padded_embeds.append(torch.cat([
                    embeds,
                    torch.zeros(pad_len, embeds.size(1), device=device)
                ], dim=0))
                ...
                padded_labels.append(torch.cat([
                    labels,
                    torch.full((pad_len,), -100, device=device)
                ], dim=0))

            return (
                torch.stack(padded_embeds).to(dtype=dtype),
                torch.stack(padded_attn),
                torch.stack(padded_labels)
            )

        def sample_zprime_tokens(zprime_dists):
            # zprime_dists: (B, T, V)
            B, T, V = zprime_dists.shape

            probs = zprime_dists.detach()  # detach so no graph is kept
            probs = probs.view(B * T, V)

            sampled = torch.multinomial(probs, num_samples=1)  # (B*T, 1)
            sampled = sampled.view(B, T)
        ...
        for epoch in range(EPOCHS):
            ...
            for i, batch in enumerate(train_loader):
                ...
                # sample z'
                embeds_z = softprompt.forward()   # [1, soft_len, dim]
                embeds_z = embeds_z.expand(input_ids_x_y.shape[0], -1, -1)
                attn_mask_z = torch.ones(
                    (input_ids_x_y.shape[0], embeds_z.shape[1]),
                    dtype=dtype,
                    device=device
                )

                embeds_zprimes, dist_zprimes = ste_generate(
                    model,
                    embeds_z,
                    attn_mask_z,
                    64,
                ...
                sampled_zprime = tokenizer.decode(sample_zprime_tokens(dist_zprimes)[0])

                embeds_x_zprime_y, attn_mask_x_zprime_y, labels_x_zprime_y = prep_inputs_for_zprime_loss_embeds(
                    input_ids_x_y,
                    labels_x_y,
                    embeds_zprimes
                )
                ...
                loss_under_zprime = manual_loss(outputs_x_p_zprime_y.logits, labels_x_zprime_y).mean()

                loss = loss_under_z + loss_under_zprime

                logger.info(
                    f"TRAIN---Epoch: {epoch}, Batch: {i}\n"
                    f"\tloss under z: {loss_under_z}\n" 
                    f"\tloss under z': {loss_under_zprime}\n"
                    f"\tsample z': {sampled_zprime}\n"
                )
                train_loss = loss
