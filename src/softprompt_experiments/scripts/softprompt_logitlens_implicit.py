...
def run(args_list):
    ...
    num = 0
    for i in range(10):
        tokenized_num = tokenizer(str(i), add_special_tokens=False).input_ids[0]
        natural_number_tokens.append(tokenized_num)
    ...
    for dataset_dir in tqdm(dataset_dirs):
        ...
        for i, idx in enumerate(random_idxs):
            ...
            for hidden_state in enumerate(hidden_states):
                ...
                # print(f"lsat token hidden: {last_token_hidden}")
                unembeddings = last_token_hidden @ only_number_unembedding_matrix.T
                # print(f"unembeddings shape: {unembeddings.shape}")
                # print(f"unembeddings: {unembeddings}")

                layer_prob = torch.nn.functional.softmax(unembeddings, dim=-1)
                layer_logprob = torch.nn.functional.log_softmax(unembeddings, dim=-1)
                # print(f"this is layer shape: {layer_prob.shape}")
                layer_probabilities.append(layer_prob)
                layer_logprobabilities.append(layer_logprob)
                # k_most_likely = 10
            ...
            def visualize_probs(layer_probas, title=f"visual_probs{i}.jpg"):
                probs_per_layer = torch.concatenate(layer_probas, dim=0)  # (num_layers, K)
                ...
                probs_to_show = probs_per_layer.detach().cpu().float().numpy()          
                ...
                plt.ylabel('Probability of decoding to this number token (first digit of full answer)')
                # plt.yticks(ticks=range(len(numbers_to_show_filtered)), labels=numbers_to_show_filtered)            
                plt.title(f"Probability distribution over numbers (first digit)\n(input= {input_text}, target = {target_number}, hardprompt = {hardprompt}")
                save_path = os.path.join(dataset_dir, title)
                plt.savefig(save_path, dpi=300)
                plt.close()
            
            visualize_probs(layer_probabilities, f"visual_probs{i}.jpg")
            visualize_probs(layer_logprobabilities, f"visual_logprobs{i}.jpg")
