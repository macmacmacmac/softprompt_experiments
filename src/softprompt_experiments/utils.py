...
def get_train_test_from_softprompts(
    ...
    if use_logits:
        ...
        for dataset_dir in dataset_dirs:
            ...
            soft_logit = softprompt.get_prompt_logits()
        ...
        with torch.no_grad():
            ...
            hardprompt_embeds = word_embeddings(tokenized_hardprompts)
            soft_embeds = torch.cat(soft_embeds, dim=0)
            full_embeds = torch.cat([soft_embeds, hardprompt_embeds], dim=1)

    labels = copy.deepcopy(full_embeds)
    labels[:,:soft_embeds.shape[1],:] = -100
